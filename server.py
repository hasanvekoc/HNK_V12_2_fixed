...
API_TOKEN = os.getenv('HNK_API_TOKEN')
PEER_TOKEN = os.getenv('HNK_PEER_TOKEN')
RATE = {}
RATE_LIMIT = int(os.getenv('HNK_RATE_LIMIT', '60'))
MEMPOOL = {}
LOCK = threading.Lock()
...
def guard(req, authorization):
    ...
    if RATE[key] > RATE_LIMIT:
        raise HTTPException(429, 'rate limit exceeded')
    if len(RATE) > 10000:
        RATE.clear()

def peer_guard(authorization):
    if PEER_TOKEN and authorization != f'Bearer {PEER_TOKEN}':
...
def http_json(url, method='GET', data=None):
    req = urllib.request.Request(
        url,
        method=method,
        headers={'Authorization': f'Bearer {PEER_TOKEN}'} if PEER_TOKEN else {}
    )
    if data is not None:
    ...
    for peer in PEERS:
        try:
            ...
            for h in range(local_h + 1, rh + 1):
                ...
                if p.get('chain_id') not in (None, _import_('core').CHAIN_ID):
                    raise ValueError('peer chain mismatch')
                if int(p.get('height', -1)) != h:
                    raise ValueError('peer height mismatch')
                expected_prev = ledger.db.execute(
                    'SELECT hash FROM blocks WHERE height=?', (h - 1,)
                ).fetchone()[0]
                if p.get('prev_hash') not in (None, expected_prev):
                    raise ValueError('peer previous hash mismatch')
                local_hash = ledger.commit_block(
                    p['proposer'], p['txs'], p['votes'], expected_height=h
                )
                if local_hash != p.get('hash'):
                    raise ValueError('peer block hash mismatch')
            local_h = rh
        except Exception:
            continue
...
def health():
    ...
    return {
        'ok': True,
        'version': VERSION,
        'chain_id': ledger.db.execute(
            "SELECT v FROM meta WHERE k='chain_id'"
        ).fetchone()[0],
        'height': h[0],
        'tip': h[1],
        'mempool': len(MEMPOOL),
...
def balance(address: str):
    if not _import_('core').valid_address(address):
...
def verify(tx: Tx, request: Request, authorization: str | None = Header(default=None)):
    ...
    try:
        return {
            'valid': True,
            'txid': txid(tx.model_dump()),
            'sender': ledger.verify_tx(tx.model_dump())['sender']
        }
    except Exception as e:
...
def submit(tx: Tx, request: Request, authorization: str | None = Header(default=None)):
    ...
    raw = tx.model_dump()
    tid = txid(raw)
    if len(MEMPOOL) >= int(os.getenv('HNK_MAX_MEMPOOL', '10000')) and tid not in MEMPOOL:
        raise HTTPException(429, 'mempool full')
    try:
        ledger.verify_tx(raw)
    except Exception as e:
        raise HTTPException(400, str(e))
    with LOCK:
        if tid in MEMPOOL:
            return {'accepted': True, 'txid': tid, 'duplicate': True}
        MEMPOOL[tid] = raw
...
def mempool(request: Request, authorization: str | None = Header(default=None)):
    guard(request, authorization)
    return {'txs': list(MEMPOOL.values())}
    ...
    if not txs and req.txids:
        with LOCK:
            for tid in req.txids:
                if tid not in MEMPOOL:
                    raise HTTPException(400, f'tx not in mempool: {tid}')
                txs.append(MEMPOOL[tid])
    try:
        h = ledger.commit_block(req.proposer, txs, req.votes)
        with LOCK:
            for t in txs:
                MEMPOOL.pop(txid(t), None)
        return {
            'committed': True,
            'height': ledger.db.execute(
...
def peer_proposal(height: int, authorization: str | None = Header(default=None)):
    ...
    for tid in p['txids']:
        x = ledger.db.execute(
            'SELECT payload FROM txs WHERE txid=?', (tid,)
        ).fetchone()
        if not x:
            raise HTTPException(409, 'transaction body unavailable')
        tx = json.loads(x[0])
        if txid(tx) != tid:
            raise HTTPException(409, 'transaction id mismatch')
        txs.append(tx)
    votes = []
    for v in p.get('votes', []):
        vr = ledger.db.execute(
            'SELECT voter,signature FROM votes WHERE block_hash=? AND voter=?',
            (r['hash'], v['voter'])
        ).fetchone()
        pk = ledger.db.execute(
            'SELECT public_key FROM validator_keys WHERE address=?',
            (v['voter'],)
        ).fetchone()
        if not pk:
            raise HTTPException(409, 'validator key unavailable')
        votes.append({
            'voter': v['voter'],
            'public_key': pk[0],
    ...
    return {
        ...
        'votes': votes,
        'hash': r['hash'],
        'chain_id': _import_('core').CHAIN_ID
    }
